...
<?php		// Declare all the variables for error messages.
  	...
  	if(!isset($_SESSION['username']))
	{
		...
		// declare all the empty variables.		
		...
		if(isset($_POST['submit']) && $valid)	// If all the fields are valid then form submitted and redirect on view.php
	    	{ 	
			// prevent an XSS or SQL injection attack.
			...
			// Object for data base connection and connecting queries.
...
<!DOCTYPE html>
	...
	<h2>	<?php header_page(); ?>	</h2>
		...
		?>
			...
			<?php 
				if($id)	// check the id if it is click on id field in view.php
				{
			?>
				<tr>
					<td>id:</td>	<!-- redirect only read only field. user can't edit that field -->
			...
			?>
				<tr>
					...
					if(isset($id))	
					{
					?>	<!-- Shows only if user click on ID fiels on view.php -->
